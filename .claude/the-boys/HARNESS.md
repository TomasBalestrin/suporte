...
**Design system:** Soldier Boy enforça o canon padrão (`tokens.json` + `registry.json` + `canon.html`) vendorizado em `.claude/the-boys/design-system/` — viaja pra todo projeto (D033). Override local: se o projeto-cliente tem `design-system/` próprio na raiz, aquele tem precedência (marca do cliente não se atropela).
